<?php $__env->startSection('title', 'Task Management | Builder360'); ?>
...
<style>
  ...
  .tm-body.pad { padding: 20px; overflow-y: auto; }
...
<section
    ...
    data-open-create="<?php echo e(($filters['create'] ?? false) || old('form_context') === 'create' ? '1' : '0'); ?>"
    data-user-id="<?php echo e(auth()->id()); ?>"
    data-company-id="<?php echo e(auth()->user()?->company_id ?? $tasks->getCollection()->first()?->company_id); ?>"
    data-task-version="<?php echo e($tasks->getCollection()->max(fn($task) => $task->updated_at?->timestamp) ?? 0); ?>"
...
>
    ...
    <aside id="taskWorkspaceRail" class="tm-rail" x-bind:class="railClasses" x-ref="taskRail" tabindex="-1" aria-label="Task workspace">
        ...
        <nav class="tm-nav">
            ...
            <?php $__currentLoopData = ['dashboard' => 'fa-table-cells-large', 'mine' => 'fa-list-check', 'assigned-to-me' => 'fa-user-plus', 'assigned-by-me' => 'fa-paper-plane', 'team' => 'fa-users', 'department' => 'fa-building', 'all' => 'fa-layer-group']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $icon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a class="tm-nav-item <?php echo e($scope === $key ? 'on' : ''); ?>" href="<?php echo e(route('collaboration.tasks.index', $taskQuery(['scope' => $key, 'page' => null]))); ?>">
                    <i class="fa-solid <?php echo e($icon); ?>" aria-hidden="true"></i><span><?php echo e($scopeLabels[$key]); ?></span>
                    <?php if($key !== 'dashboard'): ?><span class="tm-ncount"><?php echo e($taskScopeCounts[$key] ?? 0); ?></span><?php endif; ?>
                </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

            <p class="tm-nav-sec">Due & status</p>
            <?php $__currentLoopData = ['due-today' => 'fa-calendar-day', 'due-week' => 'fa-calendar-week', 'overdue' => 'fa-triangle-exclamation', 'pending' => 'fa-clock', 'completed' => 'fa-circle-check', 'archived' => 'fa-box-archive']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $icon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a class="tm-nav-item <?php echo e($scope === $key ? 'on' : ''); ?>" href="<?php echo e(route('collaboration.tasks.index', $taskQuery(['scope' => $key, 'page' => null]))); ?>">
                    <i class="fa-solid <?php echo e($icon); ?>" aria-hidden="true"></i><span><?php echo e($scopeLabels[$key]); ?></span>
                    <?php if($key === 'overdue' && ($taskScopeCounts[$key] ?? 0) > 0): ?><span class="tm-ndot"><?php echo e($taskScopeCounts[$key]); ?></span><?php else: ?><span class="tm-ncount"><?php echo e($taskScopeCounts[$key] ?? 0); ?></span><?php endif; ?>
                </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

            <?php if($canManageTasks): ?>
                <p class="tm-nav-sec">Insights & setup</p>
                <?php $__currentLoopData = ['activity' => 'fa-clock-rotate-left', 'reports' => 'fa-file-lines', 'analytics' => 'fa-chart-line', 'templates' => 'fa-clone', 'settings' => 'fa-gear']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $icon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a class="tm-nav-item <?php echo e($scope === $key ? 'on' : ''); ?>" href="<?php echo e(route('collaboration.tasks.index', $taskQuery(['scope' => $key, 'page' => null]))); ?>">
                        <i class="fa-solid <?php echo e($icon); ?>" aria-hidden="true"></i><span><?php echo e($scopeLabels[$key]); ?></span>
                    </a>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="tm-main">
        <header class="tm-compactbar">
            ...
            <div class="tm-compact-title">
                <b><?php echo e($scopeLabels[$scope] ?? 'Task Management'); ?></b>
                <span><?php echo e(number_format($tasks->total())); ?> task<?php echo e($tasks->total() === 1 ? '' : 's'); ?></span>
            </div>
            <div class="tm-compact-spacer"></div>

            <?php if(! in_array($scope, ['dashboard', 'activity', 'reports', 'analytics', 'templates', 'settings'], true)): ?>
                <div class="tm-viewseg" role="tablist" aria-label="Task view">
                    <?php $__currentLoopData = ['board' => ['fa-table-columns', 'Board'], 'list' => ['fa-list', 'List'], 'calendar' => ['fa-calendar', 'Calendar']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => [$icon, $label]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a role="tab" aria-selected="<?php echo e($view === $key ? 'true' : 'false'); ?>" class="<?php echo e($view === $key ? 'on' : ''); ?>" href="<?php echo e(route('collaboration.tasks.index', $taskQuery(['view' => $key, 'page' => null]))); ?>"><i class="fa-solid <?php echo e($icon); ?>"></i><?php echo e($label); ?></a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            ...
            <?php if($canCreateTask): ?><button type="button" class="tm-new compact tm-compact-create" x-bind:class="railOpen ? 'rail-is-open' : ''" x-on:click="openCreate"><i class="fa-solid fa-plus"></i> New task</button><?php endif; ?>
        </header>

        <?php if(session('status')): ?><div class="blade-alert blade-alert-success tm-workspace-alert"><?php echo e(session('status')); ?></div><?php endif; ?>
        <?php if($errors->any()): ?>
            <div class="blade-alert blade-alert-danger tm-workspace-alert"><strong>Check the highlighted inputs.</strong><ul><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li><?php echo e($error); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul></div>
        <?php endif; ?>

        <?php if(! in_array($scope, ['dashboard', 'activity', 'reports', 'analytics', 'templates', 'settings'], true)): ?>
            <section class="tm-options-panel" x-show="optionsOpen" x-cloak aria-label="Task options">
                <form method="GET" action="<?php echo e(route('collaboration.tasks.index')); ?>" class="tm-options-form">
                    <input type="hidden" name="scope" value="<?php echo e($scope); ?>"><input type="hidden" name="view" value="<?php echo e($view); ?>">
                    <label class="tm-search"><i class="fa-solid fa-magnifying-glass"></i><input type="search" name="q" value="<?php echo e($filters['q'] ?? ''); ?>" placeholder="Search tasks..."></label>
                    <select class="tm-select tm-filter-select" name="priority" aria-label="Priority"><option value="">All priorities</option><?php $__currentLoopData = $priorities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($value); ?>" <?php if(($filters['priority'] ?? null) === $value): echo 'selected'; endif; ?>><?php echo e($label); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select>
                    <select class="tm-select tm-filter-select" name="project_id" aria-label="Project"><option value="">All projects</option><?php $__currentLoopData = $projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($project->id); ?>" <?php if(($filters['project_id'] ?? null) == $project->id): echo 'selected'; endif; ?>><?php echo e($project->code); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select>
                    <button class="tm-tbtn" type="submit"><i class="fa-solid fa-filter"></i> Apply</button>
                    <a class="tm-tbtn" href="<?php echo e(route('collaboration.tasks.export', $taskQuery(['format' => 'csv']))); ?>"><i class="fa-solid fa-download"></i> CSV</a>
                    <a class="tm-tbtn" href="<?php echo e(route('collaboration.tasks.index', ['scope' => $scope, 'view' => $view])); ?>">Reset</a>
                </form>
            </section>
        <?php endif; ?>

        <?php if($scope === 'dashboard'): ?>
            <?php echo $__env->make('collaboration.tasks.partials.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        <?php elseif(in_array($scope, ['activity', 'reports', 'analytics', 'templates', 'settings'], true)): ?>
            <?php echo $__env->make('collaboration.tasks.partials.insights', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        <?php else: ?>
            <?php echo $__env->make('collaboration.tasks.partials.views', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        <?php endif; ?>
    </div>

    <?php echo $__env->make('collaboration.tasks.partials.create-modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php if($selectedTask): ?><?php echo $__env->make('collaboration.tasks.partials.drawer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php endif; ?>
</section>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.builder360-classic', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Mataji\Desktop\Codex1\Builder360\laravel-builder360\resources\views/collaboration/tasks/index.blade.php ENDPATH**/ ?>