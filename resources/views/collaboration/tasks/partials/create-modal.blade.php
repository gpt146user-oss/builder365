    ...
    <section class="tm-modal" role="dialog" aria-modal="true" aria-labelledby="task-create-title">
        ...
        <form method="POST" action="{{ route('collaboration.tasks.store') }}">@csrf
            ...
            <div class="tm-modal-body"><div class="tm-form-grid">
                ...
                <details class="tm-people-select full" @if($errors->has('assigned_to_user_id')) open @endif>
                    <summary><span><small>Assign to</small><b>{{ $selectedAssignee?->name ?? 'Unassigned' }}</b></span><i class="fa-solid fa-chevron-down"></i></summary>
                    <fieldset class="people-search-picker" x-data="peopleSearch"><legend class="sr-only">Choose assignee</legend><label class="tm-search people-search-input"><i class="fa-solid fa-magnifying-glass"></i><input type="search" placeholder="Search employee name, role or department..." x-on:input="filterPeople($event)" oninput="if (window.filterPeople) window.filterPeople(event);"></label><div class="people-search-results"><label data-person-search="unassigned self"><input type="radio" name="assigned_to_user_id" value="" @checked(!old('assigned_to_user_id'))><span><b>Unassigned</b><small>Assign later</small></span></label>@foreach($users as $userOption)<label data-person-search="{{ strtolower($userOption->name.' '.$userOption->email.' '.($userOption->role?->name ?? '').' '.($userOption->employee?->department ?? '')) }}"><input type="radio" name="assigned_to_user_id" value="{{ $userOption->id }}" @checked(old('assigned_to_user_id')==$userOption->id)><span><b>{{ $userOption->name }}</b><small>{{ $userOption->role?->name }} · {{ $userOption->employee?->department ?? $userOption->email }}</small></span></label>@endforeach</div></fieldset>
                </details>
