...
<details
    ...
    data-account-id="{{ $mailboxAccount->id }}"
    data-autosave-url="{{ route('mailbox.drafts.store', $mailboxAccount) }}"
    data-to='@json(old('to', $composeData['to'] ?? []))'
    data-cc='@json(old('cc', $composeData['cc'] ?? []))'
    data-bcc='@json(old('bcc', $composeData['bcc'] ?? []))'
    data-body="{{ old('body', $composeData['body'] ?? '') }}"
    data-body-html="{{ old('body_html', $composeData['body_html'] ?? '') }}"
    data-discard-url="{{ $composeDraft ? route('mailbox.drafts.destroy', [$mailboxAccount, $composeDraft]) : '' }}"
    @if($composeOpen) open @endif
>
    ...
    <div class="b360-mail-compose-panel" role="dialog" aria-modal="true" aria-labelledby="business-email-compose-title">
        ...
        <header class="b360-unified-compose-head">
            <div>
                <h2 id="business-email-compose-title">{{ $composeData['title'] ?? 'New message' }}</h2>
        ...
        <form method="POST" enctype="multipart/form-data" action="{{ route('mailbox.external.send', $mailboxAccount) }}" class="b360-unified-compose-form" x-ref="form" x-on:submit="prepareSubmit">
            @csrf
            <input type="hidden" name="client_token" value="{{ $draftToken }}">
            <input type="hidden" name="lock_version" value="{{ $composeDraft?->lock_version }}" x-ref="lockVersion">
            <input type="hidden" name="in_reply_to" value="{{ old('in_reply_to', $composeData['in_reply_to'] ?? '') }}">
            <input type="hidden" name="references" value="{{ old('references', $composeData['references'] ?? '') }}">
            ...
            <label class="b360-compose-field">
                ...
                <select x-on:change="accountChanged" aria-label="From email account">
                    @foreach($sendAccounts as $account)
                        <option value="{{ $account->id }}" data-compose-url="{{ route('mailbox.external.show', [$account, 'compose' => 'new']) }}" @selected($account->id === $mailboxAccount->id)>{{ $account->name }} · {{ $account->email }}</option>
                    @endforeach
                </select>
            </label>
            ...
            <section class="b360-email-recipient-section" aria-label="Email recipients">
                ...
                <datalist id="mailbox-contact-suggestions">
                    @foreach($contacts as $contact)
                        <option value="{{ $contact['email'] }}">{{ $contact['name'] }} · {{ $contact['source'] }}</option>
                    @endforeach
                </datalist>
            ...
            <label class="b360-compose-field">
                <span>Subject</span>
                <input name="subject" required maxlength="255" value="{{ old('subject', $composeData['subject'] ?? '') }}" placeholder="Subject">
            </label>
            ...
            <section class="b360-compose-attachment-zone" x-on:dragover.prevent="dragging=true" x-on:dragleave.prevent="dragging=false" x-on:drop.prevent="dropAttachments" x-bind:class="dragging ? 'is-dragging' : ''">
                ...
                @if($composeDraft?->attachments?->isNotEmpty())
                <div class="b360-compose-file-grid">
                    @foreach($composeDraft->attachments as $file)
                    <article>
                        <i class="fa-solid fa-file"></i>
                        <span><b>{{ $file->filename }}</b><small>{{ number_format($file->size/1024,1) }} KB</small></span>
                        <label><input type="checkbox" name="remove_attachment_ids[]" value="{{ $file->id }}"> Remove</label>
                    </article>
                    @endforeach
                </div>
                @endif
            ...
            <label class="b360-compose-field b360-send-later-field">
                <span>Later</span>
                <input type="datetime-local" name="scheduled_for" value="{{ old('scheduled_for', $composeDraft?->scheduled_for?->format('Y-m-d\TH:i')) }}">
            </label>
            ...
            <footer class="b360-unified-compose-actions">
                ...
                <button type="submit" class="blade-secondary-action" formaction="{{ route('mailbox.drafts.store', $mailboxAccount) }}" name="state" value="draft">Save draft</button>
                <button type="submit" class="blade-secondary-action" formaction="{{ route('mailbox.drafts.store', $mailboxAccount) }}" name="state" value="scheduled">Send later</button>
