...
@section('title','Drafts | '.$mailboxAccount->email)
...
<div class="dft dft-page">
  ...
  <div class="dft-head">
    <div class="dft-head-copy">
      ...
      <div class="dft-head-email">{{ $mailboxAccount->email }}</div>
    </div>
    <a class="dft-back" href="{{ route('mailbox.external.show', $mailboxAccount) }}">
  ...
  <div class="dft-list">
    @forelse($drafts as $draft)
      ...
      <article class="dft-card">
        ...
        {{-- State dot --}}
        <span class="dft-dot {{ $dotClass }}" aria-hidden="true"></span>

        {{-- Copy --}}
        <div class="dft-card-body">
          <div class="dft-subject">{{ $draft->subject ?: '(No subject)' }}</div>

          <div class="dft-recipients">
            <i class="fa-regular fa-user" aria-hidden="true" style="font-size:11px;margin-right:3px;"></i>
            {{ collect($draft->to_addresses)->join(', ') ?: 'No recipients yet' }}
          </div>

          <div class="dft-meta">
            <span class="dft-pill {{ $pillClass }}">{{ str($state)->headline() }}</span>
            ...
            @if($draft->scheduled_for)
              ...
              <span>
                ...
                Sends {{ $draft->scheduled_for->format('d M Y, h:i A') }}
              </span>
            @endif
          </div>
        ...
        {{-- Right: continue + discard --}}
        <div class="dft-card-right">
          <a class="{{ $btnClass }}"
             href="{{ route('mailbox.external.show', [$mailboxAccount, 'draft' => $draft->id]) }}">
            <i class="fa-solid {{ $isFailed ? 'fa-rotate-right' : 'fa-pen' }}" aria-hidden="true"></i>
            {!! $btnLabel !!}
          </a>

          <form method="POST"
                action="{{ route('mailbox.drafts.destroy', [$mailboxAccount, $draft]) }}"
  ...
  {{-- Pagination --}}
  @if($drafts->hasPages())
    <div class="dft-pagination">{{ $drafts->links() }}</div>
  @endif
